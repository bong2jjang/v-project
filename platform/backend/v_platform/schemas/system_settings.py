...
class SystemSettingsBase(BaseModel):
    ...
    manual_url: str = Field(default="http://127.0.0.1:3000", description="메뉴얼 URL")
    default_start_page: str = Field(
        default="/", description="시스템 기본 시작 페이지 경로"
    )
    # 앱 브랜딩
    app_title: str | None = Field(
        default=None, description="앱 표시 이름 (TopBar, Login)"
    )
    app_description: str | None = Field(
        default=None, description="앱 설명 (Login 페이지)"
    )
