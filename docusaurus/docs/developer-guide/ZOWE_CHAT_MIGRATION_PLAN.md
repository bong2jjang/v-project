...
description: Matterbridge를 Zowe Chat 영감의 자체 메시지 브리지로 전환 (4주 완성)
---
...
**작성일**: 2026-03-30
**프로젝트**: VMS Chat Ops
**목표**: Matterbridge 의존성 제거 → 자체 메시지 브리지 구현
**일정**: 4주 완성
...
| 문제 | 원인 | 해결책 |
|------|------|--------|
| Matterbridge 블랙박스 | Go 바이너리, 커스터마이징 어려움 | FastAPI로 자체 구현 |
| 설정 변경 시 재시작 필요 | TOML 기반 정적 설정 | Redis 기반 동적 라우팅 |
| 메시지 변환 제한 | Matterbridge 내장 로직 | Common Schema로 완전 제어 |
...
**Matterbridge 탈피의 핵심**: `slack_sdk.bolt`를 사용하여 Socket Mode 활성화
...
#### Day 11-14: 기존 기능 이관
- [ ] `matterbridge_message_collector.py` 로직 이관
- [ ] 파일 업로드/다운로드 처리
- [ ] 사용자 매핑 로직
- [ ] 통합 테스트
...
services:
  backend:
    ...
    environment:
      - BRIDGE_TYPE=native  # Matterbridge 대신
...
## 🎨 Matterbridge 의존성 제거 체크리스트

### 코드 변경

- [ ] **Backend Services**
  ```
  app/services/matterbridge_control.py → bridge_control.py
  app/services/matterbridge_monitor.py → bridge_monitor.py
  app/services/matterbridge_message_collector.py → (삭제, Provider로 대체)
  app/services/config_manager.py → (TOML 제거, DB/Redis 기반)
  ```

- [ ] **API Endpoints**
  ```
  app/api/matterbridge.py → bridge.py
  POST /api/matterbridge/start → POST /api/bridge/start
  GET /api/matterbridge/status → GET /api/bridge/status
  ```
...
- [ ] **Frontend**
  ```typescript
  src/store/matterbridge.ts → bridge.ts
  src/lib/api/matterbridge.ts → bridge.ts
  // UI 텍스트: "Matterbridge" → "브리지"
  ```

- [ ] **Docker Compose**
  ```yaml
  # services.matterbridge 섹션 전체 제거
  # backend 서비스만 유지
  ```
...
- [ ] **README.md**
  - Matterbridge 설치 섹션 제거
  - 자체 브리지 아키텍처 설명 추가
  - Provider 추가 방법 설명

- [ ] **ARCHITECTURE.md**
  - Common Schema 다이어그램 추가
  - Provider Pattern 설명
  - Command Processor 구조 문서화

- [ ] **API.md**
  - `/api/matterbridge/*` 엔드포인트 제거
  - `/api/bridge/*` 엔드포인트 추가
  - WebSocket API 문서화
...
### Phase 4: 전체 전환 (Day 28)
```bash
# Matterbridge 컨테이너 정지
docker-compose stop matterbridge
docker-compose rm matterbridge
...
- ✅ 메시지 처리 속도: > 100 msg/s
- ✅ 시스템 가용성: > 99.9%
- ✅ 메모리 사용량: < 512MB (Matterbridge보다 효율적)
- ✅ CPU 사용률: < 30%
...
### 롤백 계획
```bash
# 문제 발생 시 Matterbridge로 즉시 복구
docker-compose -f docker-compose.matterbridge-backup.yml up -d
```
