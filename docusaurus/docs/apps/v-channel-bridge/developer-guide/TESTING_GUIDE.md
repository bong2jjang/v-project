---
id: testing-guide
title: VMS Channel Bridge 테스트 가이드
sidebar_position: 2
tags: [guide, developer]
---

# VMS Channel Bridge 테스트 가이드

VMS Channel Bridge Light-Zowe 아키텍처의 테스트 전략, 실행 방법, 검증 체크리스트를 안내합니다.
...
# 특정 디렉토리
docker exec vms-channel-bridge-backend python -m pytest tests/adapters/ -v
docker exec vms-channel-bridge-backend python -m pytest tests/services/ -v

# 특정 파일
docker exec vms-channel-bridge-backend python -m pytest tests/adapters/test_slack_provider.py -v

# 커버리지 포함
docker exec vms-channel-bridge-backend python -m pytest tests/ -v --cov=app --cov-report=term

# HTML 커버리지 리포트
docker exec vms-channel-bridge-backend python -m pytest tests/ --cov=app --cov-report=html
```
...
# 감시 모드 (개발 시)
docker exec -it vms-channel-bridge-frontend npx vitest
```
...
# Redis 연결
docker exec vms-channel-bridge-redis redis-cli PING

# Redis Route 키 확인
docker exec vms-channel-bridge-redis redis-cli KEYS "route:*"

# Backend 로그에서 에러 확인
docker logs vms-channel-bridge-backend --tail=100 | grep -i error
```
...
```bash
# 메시지 라우팅 로그
docker logs vms-channel-bridge-backend --tail=200 | grep -i "route\|message\|bridge"

# Slack 연결 상태
docker logs vms-channel-bridge-backend --tail=50 | grep -i slack
