...
VMS Chat Ops의 Teams Provider (Graph API + Bot Framework)를 연동하기 위한 Azure Bot Service 설정 방법을 안내합니다.
...
2. Description: `VMS Chat Ops Secret`, Expires: 24 months
3. **"Add"** 클릭
...
2. 다음 정보 입력:
   - **Bot handle**: 고유한 이름 (예: `vms-chatops-bot`)
...
2. **"Messaging endpoint"** 설정:
   ...
   ```
   VMS Chat Ops의 Teams Webhook 엔드포인트를 지정합니다.
...
VMS Chat Ops에서 Teams 채널 ID는 `{teamId}:{channelId}` 형식으로 저장됩니다:
