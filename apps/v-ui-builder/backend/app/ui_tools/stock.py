...
class StockParams(BaseModel):
    symbol: str = Field(..., description="종목 티커 (예: AAPL, MSFT, 005930)")
    range: Literal["1d", "5d", "1mo", "3mo"] = Field(
        default="1mo", description="조회 기간"
    )


_POINTS_BY_RANGE: dict[str, int] = {"1d": 12, "5d": 20, "1mo": 22, "3mo": 30}
...
class StockUiTool(BaseUiTool):
    ...
    async def render(
        ...
        base = _seed(params.symbol)
        n = _POINTS_BY_RANGE[params.range]
