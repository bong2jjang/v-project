...
class SlackProvider(BasePlatformProvider):
    ...
    def __init__(
        ...
        # Slack Socket Mode가 동일 이벤트를 중복 전달하는 경우를 차단하는 LRU 캐시
        # event_ts (또는 client_msg_id) 단위로 최근 처리분을 기억해 즉시 드롭
        self._recent_event_keys: "collections.OrderedDict[str, float]" = (
            collections.OrderedDict()
        )
        self._dedup_ttl = 60.0  # seconds
        self._dedup_max_size = 5000
    ...
    def _setup_event_handlers(self):
        ...
        async def handle_message(event, say):
            ...
            try:
                ...
                # Slack Socket Mode 중복 이벤트 차단
                # 같은 event_ts(또는 client_msg_id)가 짧은 간격으로 두 번 들어오는
                # 경우를 LRU 캐시로 즉시 드롭. (소스측 재전송 / 다중 세션 흔적 방어)
                dedup_key = (
                ...
                if dedup_key:
                    ...
                    # TTL 지난 항목 정리 (앞에서부터 오래된 것)
                    while self._recent_event_keys:
                        oldest_key = next(iter(self._recent_event_keys))
                        if (
                            now_mono - self._recent_event_keys[oldest_key]
                            > self._dedup_ttl
                        ):
                    ...
                    if dedup_key in self._recent_event_keys:
                        ...
                        return
                    self._recent_event_keys[dedup_key] = now_mono
                    # 캐시 상한
                    while len(self._recent_event_keys) > self._dedup_max_size:
                        self._recent_event_keys.popitem(last=False)
