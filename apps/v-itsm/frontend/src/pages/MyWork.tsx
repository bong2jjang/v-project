/**
 * v-itsm 내 업무 통합 페이지 — v0.7.
 *
 * 전 워크스페이스에서 내게 할당된 티켓을 통합 조회. 행 클릭 시 티켓의 WS 로
 * 전역 컨텍스트를 전환한 뒤 평탄 경로(/tickets/:id) 로 이동.
 * owner_id 는 서버가 current_user 로 고정.
 */
...
import * as myWorkApi from "../lib/api/myWork";
...
export default function MyWork() {
  ...
  async function fetchList(targetPage = page) {
    ...
    setError(null);
    try {
      const params: myWorkApi.MyWorkListParams = {
        page: targetPage,
        page_size: PAGE_SIZE,
      };
      ...
      const res = await myWorkApi.listMyWorkTickets(params);
  ...
  function handleReset() {
    ...
    void (async () => {
      setLoading(true);
      try {
        const res = await myWorkApi.listMyWorkTickets({
          page: 1,
          page_size: PAGE_SIZE,
        });
        setItems(res.items);
  ...
  return (
    <>
      <ContentHeader
        title="내 업무"
        description="전 워크스페이스에서 나에게 할당된 티켓을 통합 조회합니다."
      />
        ...
        <Card>
          <CardBody>
            {loading ? (
              ...
              <EmptyState
                title="할당된 티켓이 없습니다"
                description="필터를 조정하거나 담당자로 지정된 티켓을 확인하세요."
              />
            ) : (
