...
- **아키텍처 패턴**:
  ...
  - 알림은 v-channel-bridge 의 `BasePlatformProvider` 를 호출하여 Slack/Teams/Email 로 전송.
...
- **Backend 추가 의존성(예정)**:
  ...
  - `ulid-py` 또는 `python-ulid` — 티켓 ID 생성
  - v-channel-bridge 의 provider 클라이언트 import (루트 `.env` 의 `*_BACKEND_URL` 사용)
- **Frontend 추가 의존성(예정)**:
...
- **앱 간 의존**:
  - **v-channel-bridge** — 알림 발송(Slack/Teams/Email). 루트 `.env` 의 bridge URL 로 HTTP 호출.
...
- **외부 서비스**: 직접 연동 없음. 모든 외부 채널은 bridge 를 경유.
...
2. 알림 발송 시 v-channel-bridge 를 경유하며, bridge 자체를 수정하지 않는가?
3. AI 기능 추가 시 v-ui-builder 의 provider 계약을 바꾸지 않는가?
4. 루트 `.env` 의 공용 변수만 참조하고 있는가? (앱 고유 값은 `apps/v-itsm/.env`)
5. 포털(`v-platform-portal`) AppRegistry 에 등록할 때 메타데이터가 일관적인가?
6. 설계 문서(§11 포트, §12 재사용 매트릭스) 와 실제 구현이 일치하는가?
