...
5. **Provider 설정** - (마이그레이션 전) matterbridge.toml
...
# 백업 디렉토리 생성
mkdir -p "${BACKUP_DIR}"/{env,docker,database,redis,matterbridge}
...
if [ -n "$DB_CONTAINER" ]; then
    ...
    docker compose -f docker-compose.dev.yml exec -T postgres \
        pg_dump -U vmsuser vms_chat_ops > "${BACKUP_DIR}/database/full_backup.sql"
    echo "   ✅ 전체 백업 완료 (데이터 포함)"

    # 스키마만 백업
    docker compose -f docker-compose.dev.yml exec -T postgres \
        pg_dump -U vmsuser vms_chat_ops --schema-only > "${BACKUP_DIR}/database/schema_only.sql"
    echo "   ✅ 스키마 백업 완료"
...
# 5. Matterbridge 설정 백업 (마이그레이션 전)
echo "🌉 Matterbridge 설정 백업 중..."
if [ -f "matterbridge.toml" ]; then
    cp matterbridge.toml "${BACKUP_DIR}/matterbridge/matterbridge.toml"
    echo "   ✅ matterbridge.toml 백업 완료"
else
    echo "   ℹ️  matterbridge.toml 파일이 없습니다 (마이그레이션 완료된 상태)"
fi
...
echo "💾 데이터베이스 백업 중..."
docker compose -f docker-compose.dev.yml exec -T postgres \
    pg_dump -U vmsuser vms_chat_ops | gzip > "${BACKUP_DIR}/backup.sql.gz"
...
if [ -f "${BACKUP_DIR}/database/full_backup.sql.gz" ]; then
    ...
    # DB 복구
    docker compose -f docker-compose.dev.yml exec -T postgres \
        psql -U vmsuser vms_chat_ops < /tmp/restore.sql
...
# 5. Matterbridge 복구 (필요 시)
if [ -f "${BACKUP_DIR}/matterbridge/matterbridge.toml" ]; then
    echo "🌉 Matterbridge 설정 복구 중..."
    cp "${BACKUP_DIR}/matterbridge/matterbridge.toml" matterbridge.toml
    echo "   ✅ matterbridge.toml 복구 완료"
fi
...
## 관련 스킬
- `cleanup-matterbridge` - 백업 후 Matterbridge 정리
