...
Light-Zowe 아키텍처 배포 전 필수 항목들을 자동으로 검증합니다.

**참고**: 마이그레이션 완료 후 프로덕션 배포 전에 사용하는 명령어입니다.
...
```bash
# docker-compose.yml에서 matterbridge 서비스 확인
if grep -q "matterbridge:" docker-compose.yml; then
  echo "❌ docker-compose.yml에 matterbridge 서비스가 아직 존재합니다"
  echo "   조치: matterbridge 서비스 제거 필요"
else
  echo "✅ docker-compose.yml에서 matterbridge 제거 완료"
fi

# matterbridge 컨테이너 실행 여부 확인
if docker ps -a | grep -q matterbridge; then
  echo "⚠️  matterbridge 컨테이너가 아직 존재합니다"
  echo "   조치: docker rm -f <container_name>"
else
  echo "✅ matterbridge 컨테이너 없음"
fi

# matterbridge.toml 파일 확인
if [ -f "matterbridge.toml" ]; then
  echo "⚠️  matterbridge.toml 파일이 아직 존재합니다"
  echo "   조치: 백업 후 제거 권장"
else
  echo "✅ matterbridge.toml 제거 완료"
fi
```
...
### 5. Matterbridge 제거 ✅
✅ docker-compose.yml에서 제거 완료
✅ 컨테이너 없음
⚠️  matterbridge.toml 파일 존재 (백업 후 제거 권장)
...
### 5. Matterbridge 제거 ❌
❌ docker-compose.yml에 matterbridge 서비스 존재
❌ matterbridge 컨테이너 실행 중
...
```bash
# Matterbridge 컨테이너 및 이미지 제거
docker stop vms-chatops-matterbridge 2>/dev/null
docker rm vms-chatops-matterbridge 2>/dev/null
docker rmi 42wim/matterbridge:stable 2>/dev/null

# docker-compose.yml에서 matterbridge 서비스 제거
sed -i '/matterbridge:/,/^[^ ]/d' docker-compose.yml

# matterbridge.toml 백업
if [ -f "matterbridge.toml" ]; then
  cp matterbridge.toml backups/matterbridge.toml.backup
  rm matterbridge.toml
fi

echo "✓ Matterbridge 완전 제거 완료"
```
...
curl -X POST http://localhost:8000/api/messages/test \
  ...
  -d '{
    "source": "slack",
    "channel": "#general",
    "text": "Test message from VMS Chat Ops"
  }'
...
Docker Images:
$(docker images | grep vms-chatops)
Environment:
BRIDGE_TYPE=$(grep BRIDGE_TYPE .env | cut -d'=' -f2)
EOF
